...
const TITLES = ["MISSING YOU", "STARLIGHT", "ALWAYS"];
const SUBTITLES = [
  { line1: "Across every quiet mile between us,", line2: "the sky still keeps your name." },
  { line1: "You are the softest light I know,", line2: "steady even when I can't see you." },
  { line1: "Wherever the years take us,", line2: "you stay my favourite constellation." },
];
...
export const HorizonHero = () => {
    ...
    const createNebula = () => {
      ...
      const material = new THREE.ShaderMaterial({
        uniforms: {
          ...
          color2: { value: new THREE.Color(0xff7ab0) },
          opacity: { value: 0.3 },
        },
        vertexShader: `
          varying vec2 vUv;
  ...
  // Scroll handling — measured against the hero container so the rest of the
  // page can scroll independently without distorting the camera path.
  useEffect(() => {
    const handleScroll = () => {
      ...
      const { current: refs } = threeRefs;
      if (!refs.nebula || refs.mountains.length < 4 || !refs.locations) return;
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      <div
        ref={menuRef}
        className="invisible fixed top-1/2 left-6 z-30 hidden -translate-y-1/2 flex-col items-center gap-6 md:flex"
      >
        <div className="flex flex-col gap-1.5">
          <span className="block h-px w-8 bg-foreground/50" />
          <span className="block h-px w-5 bg-foreground/30" />
          <span className="block h-px w-8 bg-foreground/50" />
        </div>
      ...
      <section className="relative z-10 flex h-screen flex-col items-center justify-center px-6 text-center">
        <h1
          ref={titleRef}
          className="invisible font-display text-[clamp(2.75rem,11vw,9rem)] leading-[0.95] font-light tracking-[0.06em] text-halo"
        >
          {splitTitle(TITLES[0])}
        </h1>
        <div ref={subtitleRef} className="invisible mt-8 space-y-2">
          <p className="subtitle-line text-sm tracking-[0.3em] text-foreground/70 uppercase sm:text-base">
            {SUBTITLES[0].line1}
          </p>
          <p className="subtitle-line text-sm tracking-[0.3em] text-foreground/50 uppercase sm:text-base">
            {SUBTITLES[0].line2}
          </p>
        </div>
      </section>
      ...
      <div
        ...
        className="invisible fixed right-6 bottom-8 z-30 flex items-center gap-4"
      >
        <span className="text-[0.6rem] tracking-[0.4em] text-foreground/50 uppercase">
          Scroll
        </span>
        <div className="h-px w-24 overflow-hidden bg-foreground/15">
          <div
            className="h-full bg-starlight transition-[width] duration-200 ease-out"
            style={{ width: `${scrollProgress * 100}%` }}
          />
        </div>
        <span className="font-mono text-[0.6rem] text-foreground/50">
      ...
      {[0, 1].map((i) => (
        ...
        >
          <h2 className="font-display text-[clamp(2.25rem,9vw,7rem)] leading-[0.95] font-light tracking-[0.08em] text-halo">
            {TITLES[i + 1]}
          </h2>
          <div className="mt-8 space-y-2">
            <p className="text-sm tracking-[0.28em] text-foreground/70 uppercase sm:text-base">
              {SUBTITLES[i + 1].line1}
            </p>
            <p className="text-sm tracking-[0.28em] text-foreground/50 uppercase sm:text-base">
              {SUBTITLES[i + 1].line2}
            </p>
          </div>
        </section>
